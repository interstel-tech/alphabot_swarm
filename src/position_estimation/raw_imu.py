...
def convert_imu_data(accel_raw, gyro_raw):
    ...
    gx = gyro_raw['gyro_x'] / GYRO_SCALE_MODIFIER
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            if accel_raw["accel_x"] is not None and gyro_raw["gyro_x"] is not None:
                converted = convert_imu_data(accel_raw, gyro_raw)

                a = converted["accel_mps2"]
                g = converted["gyro_dps"]

                print(f"Accel (m/s²): X: {a['x']:.2f}, Y: {a['y']:.2f}, Z: {a['z']:.2f}")
                print(f"Gyro  (°/s):  X: {g['x']:.2f}, Y: {g['y']:.2f}, Z: {g['z']:.2f}\n")
            else:
